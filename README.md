...
- Skipping `reflect.Value` is invalid values (often from a `nil` pointers) by default. Built-in validators can call `ErrOnNil()` to merge a `firm.ErrInvalidValue()` before skipping.
